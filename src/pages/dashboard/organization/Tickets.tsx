...
import { Search, User, Calendar, MapPin, Ticket, Loader2 } from "lucide-react";
...
const Tickets = () => {
    ...
    const fetchTickets = async () => {
      ...
      try {
        ...
        const { data: ticketsData, error: ticketsError } = await supabase
          ...
          .select(`
            *,
            events (id, title, starts_at, venue, city, status),
  ...
  const getEventStatus = (event: any) => {
    if (!event || !event.starts_at) return { label: 'Inconnu', color: 'bg-gray-100 text-gray-500' };
    const now = new Date();
    const startDate = new Date(event.starts_at);
    if (now < startDate) return { label: 'À venir', color: 'bg-blue-100 text-blue-800 border-0' };
    return { label: 'Terminé', color: 'bg-gray-100 text-gray-800 border-0' };
  };
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Gérez et téléchargez les billets de vos événements"
      />
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1">
          ...
          <div className="relative">
            ...
            <Input
              id="search"
              placeholder="Rechercher par événement, participant..."
              value={searchTerm}
        ...
        <div className="text-sm text-muted-foreground font-medium bg-gray-50 px-3 py-1.5 rounded-full shrink-0">
      ...
      <div className="grid gap-4">
        {filteredTickets.length === 0 ? (
          <EmptyState
            ...
            description={searchTerm ? "Aucun billet ne correspond à votre recherche." : "Aucun billet n'a encore été vendu pour vos événements."}
          />
        ) : (
            ...
            return (
              <Card key={ticket.id} className="overflow-hidden rounded-xl border-gray-100 shadow-sm hover:shadow-md transition-all duration-200">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-3 sm:gap-6 text-sm text-muted-foreground">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>{event?.venue || "Lieu inconnu"}{event?.city && `, ${event.city}`}</span>
                        </div>
                        <div className="flex items-center gap-2">
                ...
                <CardContent className="pt-0 pb-4">
                  <div className="flex flex-wrap items-center justify-between pt-4 border-t border-gray-50 mt-2 gap-2">
